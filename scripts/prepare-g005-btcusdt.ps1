[CmdletBinding()]
param(
    [string]$CacheDirectory = (Join-Path ([System.IO.Path]::GetTempPath()) 'crypto-trading-g005-btcusdt-v1'),
    [string]$ProvenanceLock = (Join-Path (Split-Path -Parent $PSScriptRoot) 'artifacts\strategy-evaluation\g005-btcusdt-provenance.tsv')
)
...
$baseUrl = 'https://data.binance.vision/data/spot/monthly/klines/BTCUSDT/1d'
...
for ($month = $firstMonth; $month -lt $monthAfterLast; $month = $month.AddMonths(1)) {
    ...
    $expectedRows = [int]($nextMonth - $month).TotalDays
    if ($lines.Count -ne $expectedRows) {
        throw "Archive $sourceUrl expected $expectedRows daily rows but contains $($lines.Count)."
    }

    $timestampUnit = if ($month -lt [DateTimeOffset]::Parse('2025-01-01T00:00:00Z')) {
        'milliseconds'
    }
    else {
        'microseconds'
    ...
    if ($timestampUnit -eq 'milliseconds') {
        $expectedFirst = $month.ToUnixTimeMilliseconds()
        $expectedLast = $nextMonth.ToUnixTimeMilliseconds() - 1
        $expectedFirstIso = $month.ToString("yyyy-MM-ddTHH:mm:ss.fff'Z'")
        $expectedLastIso = $nextMonth.AddMilliseconds(-1).ToString("yyyy-MM-ddTHH:mm:ss.fff'Z'")
    }
    else {
        $expectedFirst = Get-UnixMicroseconds -Value $month
        $expectedLast = (Get-UnixMicroseconds -Value $nextMonth) - 1
        $expectedFirstIso = $month.ToString("yyyy-MM-ddTHH:mm:ss.ffffff'Z'")
        $expectedLastIso = $nextMonth.AddTicks(-10).ToString("yyyy-MM-ddTHH:mm:ss.ffffff'Z'")
    }
    if ([int64]$firstFields[0] -ne $expectedFirst -or [int64]$lastFields[6] -ne $expectedLast) {
        throw "Archive endpoints do not match its frozen calendar month: $sourceUrl"
    }
    ...
    $row = [pscustomobject][ordered]@{
        ...
        expected_first_open = $expectedFirstIso
        expected_last_close = $expectedLastIso
