...
| 85 | risk 面(风险总览 / pending_reserved / committed_exposure / 恢复账本) | W4 已完成(W5+C 扩展) | `pages/risk.tsx`(`/api/v1/risk` 组合响应 `{schema_version, paper_accounts, account_risk}`:PaperAccountReadModel 全字段 + reservations 明细含对账状态 + BigInt 十进制聚合 `lib/decimal.ts`;账户风控卡呈现 durable AccountRiskReadModel:kill switch 闩锁事实用 safe-danger 且明示「不可解除」、暂停+原因、UTC 交易日+当日计数、admitted/rejected、last_rejection、open_positions 表;scopes 缺席时保留 system not_available / 「当前投影未提供」降级语义);恢复账本语义由 `/executions` 筛选「需要关注」承载;测试 risk.test.tsx |
