    ...
    <section className="space-y-6">
      <header className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          确定性虚拟网格排行(/api/v1/scanner);最后一次离线历史排行,只读、可审计,不生成订单意图
        </p>
      </header>

      <DataCard
        title="确定性虚拟网格排行"
        subtitle="网格穿越频率是波动代理,APR 与 Rating 都是估算证据,不是实时信号"
      >
            ...
            return (
              <div className="space-y-3">
                ...
                <div className="rounded-md border border-border bg-muted/30 px-3 py-2">
                  <p className="text-xs font-medium text-muted-foreground">排行政策</p>
                  <p className="mt-0.5 text-xs">
                    显式 benchmark 优先,其余按 APR 降序;并列时按 exact instrument
                    稳定排序。benchmark 是展示优先级,不是评分加成。
                  </p>
                </div>
                ...
                <p className="text-xs text-muted-foreground">
                  所有数值来自最后一次离线历史回放;不证明 scanner
                  进程仍存活,不证明行情仍然新鲜,也不是投资建议。本页没有启动、停止、重连或交易控件。
                </p>
              </div>
            );
          }}
